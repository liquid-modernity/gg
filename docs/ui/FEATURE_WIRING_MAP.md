...
| Command palette (search dialog) | Ctrl/Cmd+K site search overlay | all | none | `GG.modules.search` (`public/assets/latest/modules/ui.bucket.search.js`) + triggers in `public/assets/latest/modules/ui.js` | Triggers: `[data-gg-search]`, `[data-gg-action="search"]`, `.gg-dock__item--search`, `.gg-search-trigger`, `button[aria-label="Search"]`, `a[aria-label="Search"]`<br>Dialog host: `#gg-dialog` or `.gg-dialog-host` or `[data-gg-ui="dialog"]` | Dialog host missing<br>External imports blocked (Fuse.js/idb-keyval)<br>`GG.services.api.getFeed` missing | Press Ctrl/Cmd+K; dialog appears; typing shows results |
...
- `postcardKebab` is referenced in `GG.util.initInteractiveModules` but no module definition exists in `public/assets/latest`; any kebab menu feature is currently inactive.
- `gg-palette-host` exists in `index.prod.xml`, but no JS opens it; current Ctrl/Cmd+K wiring uses the search dialog host instead.
