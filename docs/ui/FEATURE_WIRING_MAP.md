...
- LabelTree config: `GG.store.config.labelTreeLabels` (allowlist filter), `labelTreeExclude` (optional exclude list), `labelTreeCacheTTLMin` (cache TTL in minutes).
...
- `public/assets/latest/modules/ui.bucket.listing.js`: listing modules: `GG.modules.labelTree` (paginated feed discovery + localStorage cache), `GG.modules.feed`, `GG.modules.sitemap`, `GG.modules.tagDirectory`, `GG.modules.tagHubPage`, `GG.modules.prefetch`.
...
| LabelTree (left panel) | Labels explorer with posts per label | landing (with listing hooks), listing | `labelTreeLabels`, `labelTreeExclude`, `labelTreeCacheTTLMin` | `GG.modules.labelTree` (`public/assets/latest/modules/ui.bucket.listing.js`) | `.gg-labeltree[data-gg-module="labeltree"]` with `.gg-lt__tree` | Feed API fails<br>Cache blocked<br>No listing hooks so bucket not loaded | Open left panel; labels render, expand shows up to `maxPosts` |
