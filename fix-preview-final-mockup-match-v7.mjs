...
const SHEET_LIFECYCLE_GUARD = `#!/usr/bin/env node
import fs from 'node:fs';
import path from 'node:path';
const root = process.cwd();
const failures = [];
...
const RUNTIME_OVERFLOW_GUARD = `#!/usr/bin/env node
import fs from 'node:fs';
import path from 'node:path';
const root = process.cwd();
const failures = [];
...
const STORE_MODAL_GUARD = `#!/usr/bin/env node
import fs from 'node:fs';
import path from 'node:path';
const root = process.cwd();
const failures = [];
...
function apply() {
  ...
  console.log('Applied preview final mockup-match v7.');
  console.log('Updated:');
