...
var KEY='gg_authors_dir_v1',URL='/p/author.html',cache=null,pending=null;
function resolveOne(raw,map){ var s=slugify(raw),it=s&&map?map[s]:null,name='',href=''; if(!s) return { slug:'', name:'', href:'' }; name=clean(it&&it.name?it.name:''); href=normHref(it&&it.href?it.href:'',s); if(!name) name=titleCase(s)||'Author'; return { slug:s, name:name, href:href }; }
...
function normalizePayload(payload){ var out={},i=0,item=null,key=''; if(!payload) return out; if(Array.isArray(payload)){ for(i=0;i<payload.length;i++){ item=payload[i]; if(!item||typeof item!=='object') continue; addEntry(out,item.slug||item.id||item.key||item.name,item); } return out; } if(typeof payload!=='object') return out; if(Array.isArray(payload.authors)) return normalizePayload(payload.authors); if(Array.isArray(payload.items)) return normalizePayload(payload.items); for(key in payload){ if(!Object.prototype.hasOwnProperty.call(payload,key)) continue; addEntry(out,key,payload[key]); } return out; }
...
function parseHtml(html){ if(!html||!w.DOMParser) return {}; var doc=new w.DOMParser().parseFromString(String(html||''),'text/html'); if(!doc) return {}; return parseNode(doc.querySelector('#gg-authors-dir')); }
