(function(w, d){
  ...
  function detailSlot(name){
    return infoCardPost ? qs('[data-slot="' + name + '"]', infoCardPost) : null;
  }
  
  function setDetailRow(name, on){
    var slot = detailSlot(name);
    var row = slot ? detailClosest(slot, '.gg-pi__sec, .gg-pi__metaitem') : null;
  ...
  function buildPeople(slotName, names){
    var host = detailSlot(slotName);
    var list = Array.isArray(names) ? names.filter(Boolean) : [];
    var i = 0, name = '', row = null, avatar = null, meta = null, title = null;
    ...
    for (i = 0; i < list.length; i++) {
      ...
      avatar.className = 'gg-pi__avatar';
    ...
    setDetailRow(slotName, !!host.children.length);
  }
  
  function buildChips(slotName, items){
    var host = detailSlot(slotName);
    var list = Array.isArray(items) ? items.filter(Boolean) : [];
    var i = 0, item = null, chip = null;
    ...
    for (i = 0; i < list.length; i++) {
      item = list[i];
      chip = document.createElement(item && item.href ? 'a' : 'span');
      chip.className = 'gg-pi__chip';
      chip.textContent = item && item.text ? item.text : '';
  
      if (item && item.href) chip.href = item.href;
  ...
  function buildDetailTocFromArticle(){
    var host = detailSlot('toc');
    var body = article ? (qs('.gg-post__content.post-body.entry-content', article) || qs('.post-body.entry-content', article) || qs('.entry-content', article)) : null;
    var headings = body ? qsa('h2, h3, h4', body) : [];
    var i = 0, h = null, txt = '', id = '', li = null, a = null, n = null, t = null;
    ...
    for (i = 0; i < headings.length; i++) {
      ...
      id = h.id || '';
      if (!id) {
        id = 'toc-level-' + (i + 1) + '-' + txt.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
        h.id = id;
      }
  
      li = document.createElement('li');
      li.className = 'gg-pi__tocitem gg-pi__toclvl-' + ((parseInt((h.tagName || '').slice(1), 10) || 2));
      ...
      n.className = 'gg-pi__tocnum';
      n.textContent = String(i + 1) + '.';
  
      t = document.createElement('span');
      t.className = 'gg-pi__toctext';
      t.textContent = txt;
    ...
    setDetailRow('toc', !!host.children.length);
  }
  
  function hydrateDetailInfoFromArticle(){
    var metaSvc, meta, title, summary, author, contributors, labels, tags, dateText, updatedText, readText;
  
    refreshDetailPanelRefs();
    if (!article || !infoCardPost) return;
  
    metaSvc = GG.services && GG.services.postmeta && typeof GG.services.postmeta.getFromContext === 'function'
      ? GG.services.postmeta
      : null;
  
    meta = metaSvc ? (metaSvc.getFromContext(article) || {}) : {};
  
    title = text(qs('.gg-post__title', article)) ||
      String(article.getAttribute('data-title') || '').trim() ||
      document.title ||
      '';
  
    summary = String(meta.snippet || '').trim();
    author = String(meta.author || article.getAttribute('data-author') || '').trim();
    contributors = Array.isArray(meta.contributors) ? meta.contributors.filter(Boolean) : [];
  
    labels = qsa('.gg-post__label-link[rel="tag"]', article).map(function(a){
      return {
        text: text(a),
    ...
    tags = Array.isArray(meta.tags) ? meta.tags.map(function(x){
      return {
        text: x && x.text ? x.text : '',
        href: x && x.href ? x.href : '#'
      };
    }).filter(function(x){ return x.text; }) : [];
  
    dateText = formatDetailDate(article.getAttribute('data-date') || '');
    updatedText = formatDetailDate(meta.updated || '');
    readText = formatReadTime(meta.readLabel || meta.readMin || '');
  
    setDetailText('title', title);
    setDetailText('summary', summary);
    setDetailText('date', dateText);
    setDetailText('updated', updatedText);
    setDetailText('readtime', readText);
  
    buildPeople('author', author ? [author] : []);
    buildPeople('contributors', contributors);
    buildChips('labels', labels);
    buildChips('tags', tags);
    buildDetailTocFromArticle();
  
    setDetailRow('summary', !!summary);
    setDetailRow('date', !!dateText);
    setDetailRow('updated', !!updatedText);
    setDetailRow('readtime', !!readText);
  }
...
if (isPostLayout || isDetailSurface){
  ...
  GG.modules.Comments = GG.modules.Comments || (function(){
    ...
    function ensureNodeId(node, prefix){
      var id = '';
      if (!node || !node.setAttribute) return '';
      id = cleanText(node.getAttribute('id') || node.id);
      if (id) return id;
      commentUiSeq += 1;
      id = 'gg-cmt2-' + cleanText(prefix || 'node').replace(/[^a-z0-9_-]/gi, '').toLowerCase() + '-' + commentUiSeq;
    ...
    function isDeleteCandidateNode(node){
      var action = '';
      var href = '';
      var cls = '';
      var aria = '';
      ...
      action = cleanText(node.getAttribute('data-comment-action') || node.getAttribute('data-gg-comment-action') || node.getAttribute('data-action') || '').toLowerCase();
      href = cleanText(node.getAttribute('href') || '').toLowerCase();
      cls = cleanText(node.className || '').toLowerCase();
      if (/delete|hapus/.test(action)) return true;
      if (/comment-delete|delete-comment|commentremove/.test(cls)) return true;
      if (/\/comment\/delete|\/comments\/default\/|delete/.test(href)) return true;
      aria = cleanText(node.getAttribute('aria-label') || node.getAttribute('title') || '').toLowerCase();
      txt = cleanText(node.textContent || '').toLowerCase();
      return /delete|hapus/.test(aria) || /delete|hapus/.test(txt);
    }
    ...
    function updateHeadControls(root){
      ...
      if (widthBtn) {
        ...
        widthBtn.setAttribute('aria-pressed', widthMode === 'wide' ? 'true' : 'false');
        if (panel) widthBtn.setAttribute('aria-controls', ensureNodeId(panel, 'panel'));
      ...
      if (sortBtn) {
        ...
        sortBtn.setAttribute('aria-pressed', sortMode === 'oldest' ? 'true' : 'false');
        if (list) sortBtn.setAttribute('aria-controls', ensureNodeId(list, 'thread'));
    ...
    function ensureHeadControls(root){
      ...
      if (!tools) {
        ...
        widthBtn.setAttribute('aria-pressed', 'false');
    ...
    function ensureToggleButton(comment){
      ...
      if (replies) {
        btn.setAttribute('aria-controls', ensureNodeId(replies, 'replies'));
      } else {
    ...
    function menuActionItems(pop){
      return toArray((pop && pop.querySelectorAll) ? pop.querySelectorAll('[data-gg-comment-action="copy-link"], [data-gg-comment-action="delete"], [role="menuitem"]') : [])
        .filter(function(node){
          return !!node && isVisibleNode(node);
        });
    }
    function focusMenuItem(wrap, nextIndex){
      var pop = wrap && wrap.querySelector ? wrap.querySelector('.cmt2-ctx-pop') : null;
      var items = menuActionItems(pop);
      var idx = safeNumber(nextIndex);
      if (!items.length) return false;
      if (idx < 0) idx = 0;
      if (idx > items.length - 1) idx = items.length - 1;
      try { items[idx].focus({ preventScroll: true }); } catch (_) { try { items[idx].focus(); } catch(__) {} }
      return true;
    }
    ...
    function triggerDelete(comment){
      ...
      w.setTimeout(function(){
        ...
        deleteItem = menuItems.find(function(node){
          return isDeleteCandidateNode(node);
        }) || null;
    ...
    function bindHost(host){
      ...
      host.addEventListener('keydown', function(e){
        ...
        target = e.target && e.target.closest
          ? e.target.closest('.cmt2-ctx-btn, .cmt2-ctx-pop [data-gg-comment-action]')
          : null;
        if (target && target.matches && target.matches('.cmt2-ctx-btn') && (key === 'ArrowDown' || key === 'ArrowUp')) {
          e.preventDefault();
          wrap = target.closest('.cmt2-ctx');
          pop = wrap && wrap.querySelector ? wrap.querySelector('.cmt2-ctx-pop') : null;
          if (!wrap || !pop) return;
          if (pop.getAttribute('data-gg-state') !== 'open') toggleMenu(wrap);
          items = menuActionItems(pop);
          if (!items.length) return;
          focusMenuItem(wrap, key === 'ArrowUp' ? items.length - 1 : 0);
          return;
        }
        if (target && target.matches && target.matches('.cmt2-ctx-pop [data-gg-comment-action]')) {
          wrap = target.closest('.cmt2-ctx');
          pop = wrap && wrap.querySelector ? wrap.querySelector('.cmt2-ctx-pop') : null;
          items = menuActionItems(pop);
          ...
          if (key === 'ArrowDown' && items.length) {
            e.preventDefault();
            focusMenuItem(wrap, idx < 0 ? 0 : (idx + 1) % items.length);
            return;
          }
          if (key === 'ArrowUp' && items.length) {
            e.preventDefault();
            focusMenuItem(wrap, idx < 0 ? items.length - 1 : (idx - 1 + items.length) % items.length);
            return;
          }
          ...
          if (key === 'Escape') {
            ...
            if (wrap && wrap.querySelector) {
              var btn = wrap.querySelector('.cmt2-ctx-btn');
              if (btn) {
