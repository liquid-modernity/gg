...
for(i=0;i<max;i++){ node=headings[i]; if(!node||(node.closest&&node.closest('pre,code,[hidden],[aria-hidden=\"true\"]'))) continue; tag=(node.tagName||'').toLowerCase(); if(!/^h[1-4]$/.test(tag)) continue; level=parseInt(tag.slice(1),10)||1; if(level>4) level=4; text=(node.textContent||'').replace(/\\s+/g,' ').trim(); if(!text) continue; headingId=(node.getAttribute('id')||'').trim(); href=baseHref; if(headingId) href+='#'+encodeURIComponent(headingId); out.push({ text:text, level:level, href:href }); }
return out;
}
