...
var titleLink=qs('.gg-post-card__title-link', card),href=cardHref(card),hrefFetch=normalizePostUrl(href)||href,title=cleanText(titleLink?titleLink.textContent:''),metaKey=tocCacheKey(hrefFetch),imgSrc=extractThumbSrc(card),dateNode=qs('.gg-post-card__date', card),commentsNode=qs('.gg-post-card__meta-item--comments', card),dateText=cleanText(dateNode&&dateNode.textContent?dateNode.textContent:'')||cardAttr(card,'data-date'),commentsText=cleanText(commentsNode&&commentsNode.textContent?commentsNode.textContent:'')||cardAttr(card,'data-comments'),author=extractAuthor(card),labels=extractLabels(card),excerptEl=qs('.gg-post-card__excerpt', card),quickSnippet=cleanText(excerptEl?(excerptEl.textContent||''):''),cardMeta=parseInlinePostMeta(card),authorText=cleanText(cardMeta.author||author.text),updatedText=humanDate(cardMeta.updated||cardAttr(card,'data-updated')||cardAttr(card,'data-gg-updated')),readTimeText=readMinLabel(cardMeta.readMin||'')||estimateReadTime(card),af=null,instTags=[],instContributors=[],rawTags=[],rawContributors=[];
if(!quickSnippet) quickSnippet=cardAttr(card,'data-snippet')||cardAttr(card,'data-gg-snippet');
rawTags=Array.isArray(cardMeta.tags)?cardMeta.tags:[]; instTags=rawTags.map(tagFallback).filter(function(x){ return x&&x.text; });
rawContributors=Array.isArray(cardMeta.contributors)?cardMeta.contributors:[]; instContributors=rawContributors.map(function(x){ var n=cleanText(typeof x==='string'?x:(x&&((x.name||x.text||x.slug)||''))); if(!n||(authorText&&n.toLowerCase()===authorText.toLowerCase())) return null; var w=authorFallback(n); return { text:w&&w.name||n, href:w&&w.href||'#', src:w&&w.src||'fallback' }; }).filter(Boolean);
