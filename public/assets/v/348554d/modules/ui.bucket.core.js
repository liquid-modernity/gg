...
var infoCardPost   = infoPanelRight ? qs('#gg-postinfo', infoPanelRight) : null;
var infoCardList   = infoPanelRight ? qs('.gg-editorial-preview', infoPanelRight) : null;
...
if(infoPanelRight){
  ...
  if(infoCardPost){
    infoCardPost.hidden = !showInfo;
    if(showInfo) infoCardPost.removeAttribute('inert');
    else infoCardPost.setAttribute('inert','');
  }
  if(infoCardList){
    infoCardList.hidden = true;
    infoCardList.setAttribute('inert','');
  }
  if(showInfo){
...
function isInfoOpen(){
return rightState() === 'open' && rightMode() === 'info';
}
...
if(isInfoOpen()) hideRightPanel(triggerBtn);
