...
var allowLabelTags = cfg.allowLabelTags !== false;
...
if (!out.tags.length && allowLabelTags && root && root.querySelectorAll) {
...
function parsePostMetaFromCard(card){ var svc=GG.services&&GG.services.postmeta&&typeof GG.services.postmeta.getFromContext==='function'?GG.services.postmeta:null,pm=svc&&card?svc.getFromContext(card,{ allowLabelTags:false }):{ author:'',contributors:[],tags:[],updated:'',readMin:'',snippet:'' }; if(!card) return pm||{}; pm=pm&&typeof pm==='object'?pm:{ author:'',contributors:[],tags:[],updated:'',readMin:'',snippet:'' }; if(!pm.author) pm.author=cardAttr(card,'data-author-name')||cardAttr(card,'data-author'); if(!pm.updated) pm.updated=cardAttr(card,'data-updated')||cardAttr(card,'data-gg-updated'); if(!pm.readMin) pm.readMin=cardAttr(card,'data-read-min')||cardAttr(card,'data-readtime'); if(!pm.snippet) pm.snippet=cardAttr(card,'data-snippet')||cardAttr(card,'data-gg-snippet'); if(!Array.isArray(pm.contributors)||!pm.contributors.length) pm.contributors=splitList(cardAttr(card,'data-contributors'),/\s*;\s*/); if(!Array.isArray(pm.tags)||!pm.tags.length) pm.tags=splitList(cardAttr(card,'data-tags'),/\s*,\s*/); return pm; }
...
function movedHtml(raw){ var txt=String(raw||''); return /<title>\s*Moved Temporarily\s*<\/title>/i.test(txt)||/<h1>\s*Moved Temporarily\s*<\/h1>/i.test(txt); }
function movedHref(raw,base){ var txt=String(raw||''),m=txt.match(/<a\b[^>]*\bhref\s*=\s*(["'])(.*?)\1/i),u=''; if(!m||!m[2]) return ''; u=String(m[2]||'').replace(/&amp;/gi,'&').trim(); if(!u) return ''; try{ return new URL(u,base||window.location.href).toString(); }catch(_){ return ''; } }
function fetchPostHtml(url,signal){
var abs=normalizePostUrl(url),opts={ method:'GET', cache:'no-store', credentials:'same-origin', signal:signal };
...
return window.fetch(abs,opts).then(function(res){
  ...
  var finalUrl=(res&&res.url)||abs;
  return res.text().then(function(html){ return { html:html||'', url:finalUrl }; });
}).then(function(first){
  var html=first&&typeof first.html==='string'?first.html:'',next='';
  if(!movedHtml(html)) return html;
  next=movedHref(html,(first&&first.url)||abs);
  if(!next||next===abs) return html;
  infoDebug('InfoPanel fetch moved-follow',next);
  return window.fetch(next,opts).then(function(res){
    if(!res||!res.ok) return html;
    return res.text().then(function(nextHtml){ return nextHtml||html; });
