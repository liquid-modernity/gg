...
(function(GG, w, d){
  ...
  router.handleClick = router.handleClick || function(evt){
    try {
      ...
      if (!GG.boot || GG.boot._uiReady !== true) {
        if (GG.boot && typeof GG.boot.requestUi === 'function') {
          ...
          var timer = w.setTimeout(fallback, 120);
          GG.boot.requestUi('click').then(function(){
            if (done) return;
            done = true;
            try { w.clearTimeout(timer); } catch (_) {}
            if (GG.boot && GG.boot._uiReady === true) router.navigate(url.href);
            else fallback();
          }).catch(function(){
            fallback();
          });
