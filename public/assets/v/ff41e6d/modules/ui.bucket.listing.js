...
(function(G,w,d){'use strict';if(!G||!d.querySelector('[data-gg-module="mixed-media"]'))return;function h(){try{if(G.core&&G.core.routerCtx&&G.core.routerCtx.current){var c=G.core.routerCtx.current();if(c&&c.view&&c.view!=='home')return false;}}catch(_){ }var m=d.querySelector('main.gg-main[data-gg-view]');if(m){var v=String(m.getAttribute('data-gg-view')||'').toLowerCase();if(v&&v!=='home')return false;}return true;}function i(){if(!h())return;if(G.modules&&G.modules.mixedMedia&&G.modules.mixedMedia.init){G.modules.mixedMedia.init();return;}if(!(G.boot&&G.boot.loadModule))return;G.boot.loadModule('ui.bucket.mixed.js').then(function(){if(G.modules&&G.modules.mixedMedia&&G.modules.mixedMedia.init)G.modules.mixedMedia.init();}).catch(function(){});}if(G.boot&&G.boot.onReady)G.boot.onReady(i);else if(d.readyState==='loading')d.addEventListener('DOMContentLoaded',i,{once:true});else i();})(window.GG=window.GG||{},window,document);
