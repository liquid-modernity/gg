...
(function(GG, w, d){
  ...
  // Load UI module on first interaction, or idle fallback
  var uiFired = false;
  function fireUi(){
    if (uiFired) return;
    uiFired = true;
  ...
  if (d.readyState === 'complete') scheduleIdle(function(){ if (!uiFired) fireUi(); }, isDev() ? 1500 : 4000);
  else w.addEventListener('load', function(){ scheduleIdle(function(){ if (!uiFired) fireUi(); }, isDev() ? 1500 : 4000); }, { once: true });
})(window.GG = window.GG || {}, window, document);
