...
function normalizeCacheRecord(parsed,key){ var out={ v:1, map:{} },v=1; if(!parsed||typeof parsed!=='object') return null; if(parsed.map&&typeof parsed.map==='object'){ v=asVersion(parsed.v); out.v=v; out.map=parsed.map; return out; } if(Array.isArray(parsed)) return null; v=key&&/^gg_authors_dir_v(\d+)$/.test(key)?asVersion(RegExp.$1):1; out.v=v; out.map=parsed; return out; }
...
function writeCache(record){ var key=''; try{ if(!w.sessionStorage||!record) return; key=cacheKey(record.v); w.sessionStorage.setItem(key,JSON.stringify({ v:asVersion(record.v), map:(record.map&&typeof record.map==='object')?record.map:{} })); w.sessionStorage.setItem(ACTIVE_KEY,key); }catch(_){} }
...
function resolveOne(raw,map,strict){ var s=slugify(raw),it=s&&map?map[s]:null,name='',href=''; if(!s) return { slug:'', name:'', href:'' }; if(!it&&strict) warnMissingOnce(s); name=clean(it&&it.name?it.name:''); href=clean(it&&(it.href||it.url)?(it.href||it.url):''); if(href) href=normHref(href,s); else href='/p/'+s+'.html'; if(!name) name=titleCase(s)||'Author'; return { slug:s, name:name, href:href }; }
function load(){ var fallback=readCache(); if(cache&&cache.map) return Promise.resolve(cache.map); if(pending) return pending; if(!w.fetch){ cache=fallback||{ v:1, map:{} }; return Promise.resolve(cache.map); } pending=w.fetch(URL,{ method:'GET', cache:'no-store', credentials:'same-origin' }).then(function(res){ if(!res||!res.ok) throw new Error('authors-dir'); return res.text(); }).then(function(html){ cache=parseHtml(html); if(!cache||typeof cache!=='object') cache={ v:1, map:{} }; if(!cache.map||typeof cache.map!=='object') cache.map={}; cache.v=asVersion(cache.v); writeCache(cache); return cache.map; }).catch(function(){ cache=fallback||cache||{ v:1, map:{} }; if(!cache.map||typeof cache.map!=='object') cache.map={}; return cache.map; }).finally(function(){ pending=null; }); return pending; }
function resolveMany(raw){ var list=Array.isArray(raw)?raw:[raw],slugs=[],seen={},i=0,s=''; for(i=0;i<list.length;i++){ s=slugify(list[i]); if(!s||seen[s]) continue; seen[s]=1; slugs.push(s); } if(!slugs.length) return Promise.resolve([]); return load().then(function(map){ return slugs.map(function(it){ return resolveOne(it,map,true); }); }).catch(function(){ return slugs.map(function(it){ return resolveOne(it,{},false); }); }); }
function resolve(raw){ return resolveMany([raw]).then(function(rows){ return rows[0]||resolveOne(raw,{},false); }); }
return { keyPrefix:KEY_PREFIX, key:LEGACY_KEY, activeKey:ACTIVE_KEY, url:URL, slugify:slugify, titleCase:titleCase, fallback:function(raw){ return resolveOne(raw,{},false); }, load:load, resolveMany:resolveMany, resolve:resolve };
})();
...
href=clean(row.href||'#');
if(!name) continue;
a=d.createElement('a'); a.className='gg-pi__person'; a.href=href||'#';
