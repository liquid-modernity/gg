...
(function(G,w,d){'use strict';if(!G||!d.querySelector('[data-gg-module="mixed-media"]'))return;function s(){var m=d.querySelector('main.gg-main[data-gg-surface],main.gg-main,#gg-main');var b=d.body;var v='';if(m&&m.getAttribute)v=String(m.getAttribute('data-gg-surface')||'').toLowerCase();if(!v&&b&&b.getAttribute)v=String(b.getAttribute('data-gg-surface')||'').toLowerCase();return v;}function h(){try{if(G.core&&G.core.routerCtx&&G.core.routerCtx.current){var c=G.core.routerCtx.current();if(c&&c.surface){var cs=String(c.surface||'').toLowerCase();if(cs&&cs!=='landing'&&cs!=='home')return false;}}}catch(_){ }var v=s();if(v&&v!=='landing'&&v!=='home')return false;return true;}function i(){if(!h())return;if(G.modules&&G.modules.mixedMedia&&G.modules.mixedMedia.init){G.modules.mixedMedia.init();return;}if(!(G.boot&&G.boot.loadModule))return;G.boot.loadModule('ui.bucket.mixed.js').then(function(){if(G.modules&&G.modules.mixedMedia&&G.modules.mixedMedia.init)G.modules.mixedMedia.init();}).catch(function(){});}if(G.boot&&G.boot.onReady)G.boot.onReady(i);else if(d.readyState==='loading')d.addEventListener('DOMContentLoaded',i,{once:true});else i();})(window.GG=window.GG||{},window,document);
