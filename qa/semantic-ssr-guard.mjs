...
requirePattern(indexXml, /<header\b(?=[^>]*class=['"][^'"]*\bgg-site-head\b[^'"]*['"])(?=[^>]*data-gg-module=['"]site-head['"])[^>]*>[\s\S]*class=['"][^'"]*\bgg-site-head__menu\b[^'"]*\bgg-listing-toolbar\b/is, "root listing has stable listing toolbar contract", "index.xml");
requirePattern(indexXml, /<a\b(?=[^>]*class=['"][^'"]*\bgg-site-head__profile\b[^'"]*\bgg-root-profile__card\b[^'"]*['"])(?=[^>]*href=['"]\/p\/about\.html['"])[^>]*>[\s\S]*class=['"][^'"]*\bgg-site-head__avatar\b[^'"]*\bgg-root-profile__avatar\b/is, "listing toolbar includes correctly named root profile avatar", "index.xml");
requirePattern(indexXml, /<button\b(?=[^>]*class=['"][^'"]*\bgg-site-head__filter-button\b[^'"]*\bgg-listing-toolbar__filter\b[^'"]*['"])(?=[^>]*data-gg-listing-filter-trigger=['"]true['"])(?=[^>]*aria-haspopup=['"]menu['"])(?=[^>]*aria-expanded=['"]false['"])[^>]*>/is, "listing toolbar filter trigger is accessible", "index.xml");
requirePattern(indexXml, /id=['"]gg-listing-filter-menu['"][^>]*role=['"]menu['"][\s\S]*href=['"]\/search\/label\/Case%20Notes['"][\s\S]*href=['"]#popularpost['"]/is, "listing filter menu contains initial filter set", "index.xml");
requirePattern(indexXml, /<section\b(?=[^>]*class=['"]gg-entry-list['"])(?=[^>]*id=['"]gg-entry-list['"])(?=[^>]*role=['"]list['"])[^>]*>/is, "root listing container uses section role=list", "index.xml");
...
requirePattern(indexXml, /<article\b(?=[^>]*class=['"]gg-entry-row['"])(?=[^>]*role=['"]listitem['"])[^>]*>/is, "root listing rows expose role=listitem", "index.xml");
...
requirePattern(indexXml, /<nav\b(?=[^>]*class=['"][^'"]*\bgg-detail-outline\b[^'"]*\bgg-listing-pagination\b[^'"]*['"])(?=[^>]*data-gg-module=['"]listing-pagination-dock-tail['"])(?=[^>]*data-gg-outline-mode=['"]pagination['"])[^>]*>/is, "root listing pagination uses outline-peek pagination mode", "index.xml");
requirePattern(indexXml, /data-gg-outline-mode=['"]toc['"]|<nav\b(?=[^>]*class=['"]gg-detail-outline['"])(?![^>]*data-gg-outline-mode=['"]pagination['"])/is, "detail outline remains separate from pagination mode", "index.xml");
...
if (indexXml.includes("gg-root-profilve__avatar")) fail("index.xml: misspelled gg-root-profilve__avatar must not return");
else pass("index.xml: root profile avatar class is not misspelled");
if (/background-color:\s*fbfaf4\b/i.test(indexXml)) fail("index.xml: invalid background-color fbfaf4 must not return");
else pass("index.xml: invalid fbfaf4 color is absent");
if (countMatches(indexXml, /\bid=['"]gg-preview-taxonomy['"]/g) === 1 && countMatches(indexXml, /\bid=['"]gg-preview-taxonomy-items['"]/g) === 1) {
  pass("index.xml: preview taxonomy IDs remain unique");
} else {
  fail("index.xml: preview taxonomy IDs must remain unique");
}
