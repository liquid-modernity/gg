...
if (!rootSchemaMatch) {
  ...
  requireIncludes(rootSchema, "&quot;@type&quot;: &quot;BreadcrumbList&quot;", "root schema has route-truth BreadcrumbList", "index.xml");
  requireIncludes(rootSchema, "&quot;name&quot;: &quot;Home&quot;", "root schema breadcrumb starts with Home", "index.xml");
  requireIncludes(rootSchema, "&quot;item&quot;: &quot;https://www.pakrpp.com/landing&quot;", "root schema breadcrumb Home points to /landing", "index.xml");
  requireIncludes(rootSchema, "&quot;name&quot;: &quot;Blog&quot;", "root schema breadcrumb includes Blog", "index.xml");
  requireIncludes(rootSchema, "&quot;item&quot;: &quot;<data:blog.homepageUrl/>&quot;", "root schema breadcrumb Blog points to Blogger homepage", "index.xml");

  if (rootSchema.includes("data:schemaPosts")) {
    fail("index.xml: root schema must not use data:schemaPosts because it can crash Blog1 on live Blogger");
  } else {
    pass("root schema avoids data:schemaPosts live-Blogger crash vector");
  }

  if (/data:posts\s+filter\s*\(p\s*=&gt;/i.test(rootSchema)) {
    fail("index.xml: root schema must not build a dynamic filtered ItemList from data:posts until proven Blogger-safe");
  } else {
    pass("root schema avoids dynamic filtered data:posts ItemList");
  }

  if (rootSchema.includes("&quot;@type&quot;: &quot;ItemList&quot;")) {
    warn("index.xml: root ItemList is present; keep it only if live Blogger proves Blog1-safe");
  } else {
    pass("root schema safely defers dynamic ItemList; Blog + BreadcrumbList is the root contract");
  }

  for (const item of manifestItems) {
    if (rootSchema.includes(item.name) || rootSchema.includes(item.slug)) {
      fail(`root schema block must not hard-code Store product in Blog schema: ${item.name}`);
    }
  }
...
requireIncludes(indexXml, "data:post.body contains &quot;gg-store-data&quot;", "root visible listing excludes Store data marker", "index.xml");
requireIncludes(indexXml, "data:post.body contains &quot;gg-yellowcard-data&quot;", "root visible listing excludes Yellow Cart data marker", "index.xml");
