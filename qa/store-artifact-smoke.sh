...
check_repo_pattern '"store:check:dev10"[[:space:]]*:[[:space:]]*"npm run store:check:ci && node tools/cloudflare-prepare\.mjs && bash qa/store-artifact-smoke\.sh"' "package.json" "package.json missing final development Store gate script"
