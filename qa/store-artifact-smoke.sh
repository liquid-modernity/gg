...
check_repo_pattern '"store:check:dev10"[[:space:]]*:[[:space:]]*"npm run store:check:ci && npm run build && bash qa/store-artifact-smoke\.sh && npm run gaga:verify-sheet-contract"' "package.json" "package.json missing final development Store gate script"
