...
target_store_js="${target_root}/assets/store/store-core.js"
target_store_discovery_js="${target_root}/assets/store/store-discovery.js"
target_store_legacy_js="${target_root}/assets/store/store.js"
...
check_file_exists "$target_store_js" "store core runtime asset is missing: ${target_store_js}"
check_file_exists "$target_store_discovery_js" "store discovery runtime asset is missing: ${target_store_discovery_js}"
check_file_exists "$target_store_legacy_js" "store legacy runtime alias is missing: ${target_store_legacy_js}"
...
check_not_stale "assets/store/store-core.js" "$target_store_js" "store core runtime asset"
check_not_stale "assets/store/store-discovery.js" "$target_store_discovery_js" "store discovery runtime asset"
check_not_stale "assets/store/store.js" "$target_store_legacy_js" "store legacy runtime alias"
...
check_repo_pattern '"store:check:ci"[[:space:]]*:[[:space:]]*"GG_STORE_MODE=ci STORE_CI=1 STORE_REQUIRE_LIVE_FEED=0 STORE_STRICT_IMAGES=0 npm run store:check"' "package.json" "package.json missing clean CI Store check mode script"
check_repo_pattern '"store:check:dev10"[[:space:]]*:[[:space:]]*"npm run store:check:ci && node tools/cloudflare-prepare\.mjs && bash qa/store-artifact-smoke\.sh && npm run gaga:verify-sheet-contract"' "package.json" "package.json missing final development Store gate script"
check_repo_pattern '"gaga:verify-sheet-contract"[[:space:]]*:[[:space:]]*"bash qa/sheet-contract-smoke\.sh"' "package.json" "package.json missing sheet contract verification script"
check_repo_pattern '"store:check:production"[[:space:]]*:[[:space:]]*"GG_STORE_MODE=production STORE_CI=1 STORE_PRODUCTION=1 STORE_REQUIRE_LIVE_FEED=0 STORE_STRICT_IMAGES=1 npm run store:check"' "package.json" "package.json missing production Store check mode script"
check_repo_pattern '"gaga:verify-store-artifact"[[:space:]]*:[[:space:]]*"npm run store:check:dev10"' "package.json" "package.json missing deterministic Cloudflare artifact verification chain"
...
check_file_pattern 'function loadDiscoveryRuntime\(' "$target_store_js" "store core runtime is missing lazy Discovery loader"
check_file_pattern 'store-discovery\.js' "$target_store_js" "store core runtime is missing Discovery script path"
check_file_pattern 'data-store-discovery-runtime' "$target_store_js" "store core runtime is missing Discovery script marker"
check_file_pattern 'STORE_LCP_PRODUCT_START' "$target_store_discovery_js" "store discovery runtime asset is missing STORE_LCP_PRODUCT_START marker"
check_file_pattern 'function readStaticProducts\(' "$target_store_discovery_js" "store discovery runtime asset is missing readStaticProducts helper"
check_file_pattern 'function hydrateStaticProducts\(' "$target_store_discovery_js" "store discovery runtime asset is missing hydrateStaticProducts helper"
check_file_pattern 'function loadProducts\(' "$target_store_discovery_js" "store discovery runtime asset is missing loadProducts helper"
check_file_pattern '/store/data/manifest\.json' "$target_store_discovery_js" "store discovery runtime asset is missing discovery manifest fetch path"
check_file_pattern 'function loadStoreManifest\(' "$target_store_discovery_js" "store discovery runtime asset is missing loadStoreManifest helper"
check_file_pattern 'function validateStoreManifest\(' "$target_store_discovery_js" "store discovery runtime asset is missing manifest validator"
check_file_pattern 'function applyDiscoveryFilters\(' "$target_store_discovery_js" "store discovery runtime asset is missing manifest-backed Discovery filters"
check_file_pattern 'function fallbackDiscoveryItems\(' "$target_store_discovery_js" "store discovery runtime asset is missing Discovery static fallback"
check_file_pattern 'storeManifestCache' "$target_store_discovery_js" "store discovery runtime asset is missing in-memory manifest cache"
check_file_pattern 'state\.feedSource = ["'"'"']static["'"'"']' "$target_store_discovery_js" "store discovery runtime asset is missing static feedSource boot path"
check_file_pattern 'syncRequestedPreview\(\);' "$target_store_discovery_js" "store discovery runtime asset is missing static deep-link sync"
check_file_pattern 'function updateItemListJsonLd\(' "$target_store_discovery_js" "store discovery runtime asset is missing ItemList JSON-LD updater"
check_file_pattern 'function setCardImagePriority\(' "$target_store_discovery_js" "store discovery runtime asset is missing image priority helper"
check_file_pattern 'var loading = index === 0 \? ["'"'"']eager["'"'"'] : ["'"'"']lazy["'"'"']' "$target_store_discovery_js" "store discovery runtime asset is missing card loading priority contract"
check_file_pattern 'var fetchPriority = index === 0 \? ["'"'"']high["'"'"'] : ["'"'"']auto["'"'"']' "$target_store_discovery_js" "store discovery runtime asset is missing card fetchpriority contract"
check_file_pattern 'button\.href = canonicalProductUrl\(item\) \|\| productStoreAbsoluteUrl\(item\)' "$target_store_discovery_js" "store discovery runtime asset is missing card href canonical fallback binding"
check_file_pattern 'event\.preventDefault\(\);' "$target_store_discovery_js" "store discovery runtime asset is missing preview interception preventDefault calls"
check_file_pattern 'marketplaceAriaLabel\(' "$target_store_discovery_js" "store discovery runtime asset is missing marketplace aria-label helper"
check_file_pattern 'data-store-remove-saved' "$target_store_discovery_js" "store discovery runtime asset is missing saved remove hook"
check_file_pattern 'store-core\.js' "$target_store_legacy_js" "store legacy runtime alias does not load store-core.js"
...
if ! node -e '
  ...
  (async () => {
    ...
    const checkPage = (key, label, title, pageNumber, totalPages, totalProducts) => {
      ...
      if (!html.includes("href=\"/assets/store/store.css\"") || !/src="\/assets\/store\/store-core\.js"[^>]*data-store-discovery-src="\/assets\/store\/store-discovery\.js"[^>]*defer/.test(html)) process.exit(15);
